...
void main() {
  group('PolicyRepositoryImpl', () {
    test(
      'addPolicy refreshes reminders, enqueues records, and schedules upcoming notifications',
      () async {
        ...
        final clients = FakeClientLocalDataSource(
        ...
        final created = await repository.addPolicy(
          testPolicy(clientId: client.id),
        );
    ...
    test(
      'updateRenewalStatus queues a policy update and preserves renewal intent',
      () async {
        final currentUser = testUser();
    ...
    test(
      'deletePolicy cancels existing reminders and enqueues delete records',
      () async {
        final currentUser = testUser();
        final existing = testPolicy();
        ...
        expect(
          scheduler.cancelledReminderBatches.single.single.id,
          oldReminder.id,
        );
