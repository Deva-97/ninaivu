...
void main() {
  group('ClientRepositoryImpl', () {
    test(
      'addClient writes locally, enqueues create, and triggers best-effort sync',
      () async {
    ...
    test(
      'updateClient preserves ownership metadata and enqueues pending update',
      () async {
        final currentUser = testUser();
    ...
    test(
      'deleteClient soft deletes locally and enqueues pending delete',
      () async {
        final currentUser = testUser();
        final existing = testClient();
        final local = FakeClientLocalDataSource(
