...
void main() {
  group('UserRepositoryImpl', () {
    test('createAgent saves locally, enqueues create, and triggers sync', () async {
    ...
    test('updateCurrentUserProfileImage enqueues a minimal update payload', () async {
      final currentUser = testUser();
      ...
      expect(queue.enqueuedItems.single.payload, containsPair('id', currentUser.id));
      expect(queue.enqueuedItems.single.payload, contains('updated_at'));
      expect(queue.enqueuedItems.single.payload, containsPair('sync_status', 'pending_update'));
      expect(sync.syncPendingDataBestEffortCallCount, 1);
    });
  });
}
